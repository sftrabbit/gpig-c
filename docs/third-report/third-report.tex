...
In this way, we have focussed mostly on the quality attributes that pertain to the functionality required by a HUMS---that is, sensing, storing, analysing, and reporting---and less on those quality attributes required for real-world application (such as security). Doing so has allowed us to demonstrate the user-facing behaviour and feasability of the HUMS, while later development can tackle the details of Consumer deployment.
