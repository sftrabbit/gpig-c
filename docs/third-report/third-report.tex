...
\label{sec:views}
\hl{TODO Tom}
The Module View, as shown in Figure~\ref{fig:modules}, describes our architecture in terms of sub-components, each encapsulated so as to minimise the propagation of changes through the system.

% Previously: no engines shown, which are implemented or not
% Prev: No internals of data abstraction layer. No databases or clear interface.
% Green => stuff to be added by the End User. Implements X interface, e.g. EmitterInterface
% Within Blue => What we've implemented
% Labelled types of the components: layers, engines, emitters
% Patterns - System Data Gateway pattern to allow us to add new databases increasing modifiability as implementation is hidden behind the storage/analysis etc interface
% Observer/subscriber pattern. Controllers=observers, Engines subscribe to the appropriate observers
% Shows the boundary between what we implement and what the End User implements. We have defaults, showing what our system can do (tailorability) and leaving space for the user to add more. Code interfaces: class names etc, controllers and generic interfaces. Latex description list thing. Reference code here.
% Quality attributes?
...
The demonstration purposes, and to make our prototype easier to test and use, we created an \emph{Emitter Launcher}, which simplifies the process of running our desktop Data Emitters by allowing the individually packaged emitters to all be run from a single application, using a GUI. A screenshot of this GUI is shown in Figure~\ref{fig:launcherGUI}, it has again been created using the \emph{Standard Widget Toolkit} \hl{ref}, meaning native components can be used.
...
The traffic analysis engine, simple creates an event when a reading is generated. The earthquake engine checks if the magnitude of the earthquake is above a certain value, so larger earthquakes generate events. For other analysis engines, the analysis system can be easily built up. Hence, it is easy to add further analysis engines to the system. This can easily be done by extending the AnalysisEngine base class, and implementing the required methods.
