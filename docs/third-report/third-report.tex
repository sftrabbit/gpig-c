...
External dependancies are necessary occurrences which must take place before the HUMS can be run successfully by consumers. The previous report did not consider the external dependancies of the consumer running the HUMS rather than the development process, so they are considered here.
...
\label{sec:views}
The Module View, as shown in Figure~\ref{fig:modules}, describes our architecture from a structural perspective, in terms of sub-components, each encapsulated so as to minimise the propagation of changes through the system. % Structural perspective
...
\xtableformat{| p{1.6cm} | p{4.4cm} | p{8.6cm} |}{ \hline	FR.x  & 	Description of FR  & 	Validation \\ \hline
	...
	FR.2  & 	  & 	Timestamps are held for all data, and stored in the Datastore when the information is saved. \\ \hline
...
\xtableformat{| p{1.6cm} | p{4.4cm} | p{8.6cm} |}{ \hline	NFR.x  & 	Description of NFR  & 	Validation \\ \hline
	NFR.1  & 	  & 	The Datastore is not within the HUMS hardware, but is instead user defined. Hence, the system is capable of keeping data persistently even when the hardware the HUMS is operated on is changed. \\ \hline
