...
\xtableformat{ p{1.3cm}  X X }{ 
	...
	FR.3  & To meet the requirement to store correctly structured data, we have enforced checks on data entering the core. The data is stored in a Datastore. This is H2 for a local Datastore, and is a PostgreSQL instance on Heroku for the cloud solution. 	  & Our automated tests and assertions regarding the Datastores all passed. \\ 
	FR.4  & Manual inspection testing of the configuration method. & Inspection showed the use of JSON configuration files that can be loaded and modified at runtime, either via the administration centre or by manually modifying the file. The Core is automatically updated even if the modifications happen whilst the system is running. \\ 
