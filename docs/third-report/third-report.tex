...
External dependencies are requirements outside of the scope of control of the development project necessary for the HUMS to be run successfully. The previous report did not consider the external dependencies pertaining to the Consumer running the HUMS, as well as those of the development process, so they are considered here.
 ...
 \item[\ed{2}] Any necessary network or firewall configuration must be performed to allow Clients to communicate with the Core where necessary.
  \item[\ed{3}] The selected remote datastores must have a high enough availability to meet the non-functional requirements of the HUMS set out earlier.
 \end{description}
...
The implementation of the graph database, Neo4j, provided numerous challenges and therefore proved time consuming. As the teams data modelling experience lay largely in the relational and key-value data modelling realm, the mindset of modelling data as graphs proved challenging initially. In hindsight, more time should have been devoted to the implementation of unfamiliar datastore technologies.
