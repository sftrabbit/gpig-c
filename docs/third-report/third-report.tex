...
Table~\ref{tab:functionalEval} shows which functional requirements we were able to meet with our prototype, describing how we verified these requirements were met if verification was not completed through testing, as previously detailed. Where requirements have not been met, an explanation has been given as too why this was the case, and how in this future they would be achieved.
Table~\ref{tab:nonfunctionalEval} details the same information, for the non-functional requirements.

\xtableformat{ p{2cm}  p{2cm} X}{ 
...
\fr{4} & Partial  & Some of the refinements of this requirement were verified through testing, and a snapshot of the configuration file format used is shown in Figure~\ref{fig:configFiles}. Other refinements, concerning storage limits, could not be verified for reasons explained below. \\
\fr{5} & No & Due to timing constraints, we chose not to implement storage quotas, for our SaaS distribution of the HUMS, as part of this prototype. In the future this requirement would be verified by defining a storage limit through the Admin Centre and attempting to store more data and confirming a notification is sent and received.\\
...
\fr{10}& Yes & Unit testing, and the existence of our web based reporting tools, verifies this requirement.\\
...
\fr{12}& Partial & Dynamic class loading is fully functional, and verified through testing. However, we have not implemented a secure user system as part of this prototype due to timing constraints, meaning any user could add or remove engine files. \\
}{Verification of functional requirements, and reasons for those not met by the prototype }{tab:functionalEval}


\xtableformat{ p{2cm}  p{2cm} X}{ 
Requirement & Verified? & Reasoning \\ \hline
}{Verification of non-functional requirements, and reasons for those not met by the prototype }{tab:nonfunctionalEval}
...
Whilst we didn't have time to implement a complete solution---notably absent are storage limits---we believe that the prototype solution is a promising start towards a more complete system, and the ease with which we were able to add new engines demonstrates that it is not difficult to extend. With this in mind, the lack of some requirements should not be seen as a weakness of the system, but rather that our attention was focussed towards our core requirements of tailorability and ease of extension, from which additional requirements can easily be implemented.

A number of development tasks would have been performed differently with sufficient foresight. For example, build tools should have been determined and configured at an earlier stage, allowing more of the later work to focus on development of the HUMS itself. This would also have allowed the use of continuous integration for greater quality control and more efficient development.
