...
\fr{4.1.1}, \fr{4.1.2}, and \fr{4.1.3} were added as we implemented configuration files into our prototype. We chose to use JSON as the configuration format as it is a widely-used standard.

\fr{12} became apparent once we considered the reloading of configuration in \fr{4.1.3}, as without the ability to load new engines, adding new analyses and reporting methods would require a restart of the System to take effect.
...
\requirement{\fr}{4}{The HUMS shall store End User configuration files.}{
	\requirement{\fr}{4.1}{The HUMS shall allow authorised Users to modify configuration files.}{
      \requirement{\fr}{4.1.1}{The configuration file shall be human editable.}{}
      \requirement{\fr}{4.1.2}{The Admin Centre shall provide an interface to allow authorised users to modify the configuration.}{}
      \requirement{\fr}{4.1.3}{The HUMS shall monitor the configuration for changes and apply them.}{}
    }
...
The input to the HUMS, in this view, is the sensor data from the consumer system,
\begin{figure}
\includegraphics[width=\textwidth]{images/pipesAndFilters.pdf}
\label{fig:pipesAndFilter}
\caption{A Component and Connector Diagram of our HUMS, showing the protocols used when passing data between filters}
\end{figure}
...
The Core is the main application that controls the behaviour and functionality of the HUMS. It receives the input data from the Data Emitters and stores it using the data storage engines, then passes it on for appropriate analysis using the analysis engines, which in turn may create notifications that the Core passes on to the relevant notification engines. The different engines are all interchangeable and dynamic class loading is used to flexibly discover which engines exists and should be used by the HUMS.

Several example engines are provided with the HUMS and are described throughout this report. The engines implement the standard interfaces which have been created for the main different types of engines: storage, analysis and notification. The Customer and Consumer can easily extend and customise the system by creating their own engines to meet their own needs.

An instance of a HUMS can be defined using a configuration file. This specifies the parameters of the instance such as System IDs and the Senors involved, along with options for each sensor, for analysis, and for notification. The engines and data store to use are also specified. A configuration file is loaded into the Core by the User before it is run. Changes to configuration files can be made manually through a text editor or in a more user-friendly manner through the Admin Centre.

Any changes made to the configuration file being used by an instance of the HUMS are automatically detected by the Core and the configuration is reloaded to take immediate effect. This saves extra work for the User who may forget to reload the file, or not realise that this needs to be done or how to do it.
...
To explore the possibilities surrounding monitoring remote systems over a network, we created an Android application with three test systems. Screenshots of the Android application are shown in Figure~\ref{fig:android}.
...
We devised a generic expression engine, \texttt{ExpressionAnalysisEngine}, which allows the configuration file to describe a wide range of potential analysis processes without the need to modify any of the analysis engines. The expression engine supports most reasonable closed-form expressions, as well as less common, but helpful features such as conditional expressions. Figure~\ref{fig:exprExample} gives an example of an expression that can be given in the system configuration file to define the action of the engine.
\begin{figure}[tbp]
\centering
\verb+if(max(ENGINE_TEMP_A, ENGINE_TEMP_B) >= 120 and RPM > 5000, -1, 0)+
\caption{A basic example of an expression that evaluates to $-1$ when a system is operating outside of some defined normal operational parameters.}
...
The quality attributes laid out in Section \hl{sec} have been largely satisfied. The requirement that the HUMS system be utilisable in multiple domains was emphasised strongly, and so high flexibility was a goal from the beginning of the project. 
