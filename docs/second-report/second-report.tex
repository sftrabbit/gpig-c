...
company or individual to, for a fee, use the facilities provided; including 
...
\subsection{Source}
\label{sec:source}
The source for our prototype implementation can be accessed online at \verb+http://git.gpigc.co.uk+ and cloned using Git from \verb+http://git.gpigc.co.uk/gpig-c.git+. The username required is \verb+gpigc+ with the password hint being ``\verb+It's right under your nose+'', and the password itself being ``\verb+moustache+''.
...
% -> highlight changes in development plan - needs to be "appropriately reviewed and revised when necessary"
