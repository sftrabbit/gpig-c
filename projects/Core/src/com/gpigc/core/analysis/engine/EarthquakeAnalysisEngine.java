		...
		if (system.hasSensorWithID(EARTHQUAKE_SENSOR_ID)) {
			try {
				...
				if (system.getSensorWithID(EARTHQUAKE_SENSOR_ID)
						.getParameters().containsKey(Parameter.LOWER_BOUND)) {
					...
					if (magnitude >= lowerBound) {
						...
						data.put(Parameter.LONG,
								sensorState.getValue().split(",")[2]);
						data.put(Parameter.LAT,
								sensorState.getValue().split(",")[1]);
