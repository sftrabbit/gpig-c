...
public abstract class Controller {
	...
	protected final List<? extends Object> instantiateEngines(List<ClientSystem> allSystems, Class<?>... constructorParams) {
	...
	private final Object makeEngine(List<ClientSystem> allSystems, Constructor<?> constructor, String name) throws InstantiationException, IllegalAccessException,
			IllegalArgumentException, InvocationTargetException {
		switch (engineType) {
		case analysis:
			AnalysisEngine analEngine = (AnalysisEngine) constructor.newInstance(getRegisteredSystems(name, allSystems), core);
			return analEngine;
		case notification:
			NotificationEngine noteEngine = (NotificationEngine) constructor.newInstance(getRegisteredSystems(name, allSystems), 30); // XXX
																																		// Could
																																		// have
																																		// this
																																		// as
			...
			return noteEngine;
		case storage:
			SystemDataGateway storageEngine = (SystemDataGateway) constructor.newInstance(getRegisteredSystems(name, allSystems));
	...
	protected abstract List<ClientSystem> getRegisteredSystems(String name, List<ClientSystem> allSystems);
}
