...
public abstract class Controller {
			...
			for (File engineFile : folder.listFiles()) {
				if (engineFile.getName().endsWith(".class") && !engineFile.getName().contains("$")) {
					...
					String engineBinaryName = GPIGC_CORE_PACKAGE + "."
							+ engineType + ".engine." + className;
					Class<?> engineClass = cl.loadClass(engineBinaryName);
					try {
						Constructor<?> constructor = engineClass
	...
	private final Object makeEngine(List<ClientSystem> allSystems,
		...
		switch (engineType) {
		case analysis:
			AnalysisEngine analEngine = (AnalysisEngine) constructor
					.newInstance(getRegisteredSystems(name, allSystems), core);
			return analEngine;
		case notification:
			NotificationEngine noteEngine = (NotificationEngine) constructor
					.newInstance(getRegisteredSystems(name, allSystems), 30); // XXX
																				// Could
																				// have
																				// this
																				// as
