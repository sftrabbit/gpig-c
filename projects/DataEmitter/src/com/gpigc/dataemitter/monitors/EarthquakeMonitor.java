...
public class EarthquakeMonitor {
	...
	public List<Earthquake> getNewEarthquakes() {
		...
		try {
			...
			JSONObject featureCollection = (JSONObject) JSONValue.parse(response.toString());
			...
			for (Object featureObject : features) {
				...
				if (time > lastEarthquakeTime) {
					Earthquake earthquake = new Earthquake(time, mag, latitude, longitude);
	...
	public class Earthquake {
		...
		public Earthquake(long time, double magnitude, double latitude, double longitude) {
			this.time = time;
